...
public class Pedidos implements Serializable {
	...
	public List<Pedido> filtrados(PedidoFilter filtro){
		...
		Criteria criteria = session.createCriteria(Pedido.class)
				...
				//fazemos uma associação (join) com Vendedor e nomeamos com "v"
		...
		if(filtro.getNumeroAte() != null) {
			//id deve ser maior ou igual (le = lower or equals) a filtro.numeroAte
		...
		if(filtro.getStatuses() != null && filtro.getStatuses().length > 0) {
			//adiocionamos uma restrição "in" passando um array de constantes da enum StatusPedido
